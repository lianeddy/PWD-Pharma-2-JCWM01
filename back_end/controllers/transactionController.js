...
module.exports = {
    getTransaction: (req, res) => {
        ...
        pharma2.transaction where id_user = ${db.escape(req.params.id)} group by date  order by date asc limit ${req.params.page}, 5; `
      ...
      getTransactionFilter: (req, res) => {
        let scriptQuery = `SELECT id_transaction,
            ...
            status,
            total_price,
            (total_price + tax + shipping_cost ) as final_price FROM pharma2.transaction where id_user = ${db.escape(req.params.id)} and status = ${db.escape(req.params.status)} group by date  order by date asc  ; `;
