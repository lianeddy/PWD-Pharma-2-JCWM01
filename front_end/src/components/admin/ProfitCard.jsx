...
class ProfitCard extends React.Component {
  ...
  countRestockPriceMonthly = () => {
    ...
    .then(res => {
      this.setState({ restockPriceMonthly: parseInt(res.data.results[0].restcok_price) })
      console.log(res.data)
    })
    .catch(err => {
      ...
      <div className="mx-5 my-5">
        <div className="card text-white bg-primary mb-3" style={{ width:"17rem", maxWidth: "18rem" }}>
          ...
          <div className="card-body text-dark bg-light">
            <h5 className="card-text">Rp {((this.state.totalPriceMonthly + this.state.shippingMonthly) - (this.state.shippingMonthly + this.state.taxMonthly)).toLocaleString("id")} </h5>
          </div>
        </div>
      </div>
    );
